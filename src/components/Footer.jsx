    ...
    <footer className="bg-[#080b18] text-white">
      ...
      <div className="bg-white/[0.03] border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-white/50 text-xs tracking-wide uppercase">Ready to book your stay?</p>
          <div className="flex gap-2">
            ...
            <a href={`https://wa.me/91${whatsapp.replace(/\D/g,'').replace(/^0+/,'')}?text=Hi, I'd like to book a room at Hotel Maa Sharda Palace.`}
              ...
              className="bg-green-500 text-white px-5 py-2 rounded-full font-bold text-xs hover:bg-green-600 transition">
              💬 WhatsApp
            </a>
          </div>
        </div>
          ...
          <div className="flex gap-2.5">
            {instagram && (
              <a href={instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram"
                className="w-8 h-8 bg-white/8 rounded-full flex items-center justify-center hover:bg-gradient-to-br hover:from-pink-500 hover:to-purple-600 transition-all text-white/60 hover:text-white">
            ...
            {facebook && (
              <a href={facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook"
                className="w-8 h-8 bg-white/8 rounded-full flex items-center justify-center hover:bg-blue-600 transition-all text-white/60 hover:text-white">
            ...
            {gmb && (
              <a href={gmb} target="_blank" rel="noopener noreferrer" aria-label="Google Maps"
                className="w-8 h-8 bg-white/8 rounded-full flex items-center justify-center hover:bg-red-500 transition-all text-white/60 hover:text-white">
                <svg fill="currentColor" viewBox="0 0 24 24" className="w-3.5 h-3.5"><path d="M12 0C7.802 0 4 3.403 4 7.602 4 11.8 7.469 16.812 12 24c4.531-7.188 8-12.2 8-16.398C20 3.403 16.199 0 12 0zm0 11c-1.657 0-3-1.343-3-3s1.343-3 3-3 3 1.343 3 3-1.343 3-3 3z"/></svg>
              </a>
            )}
          </div>
        </div>
          ...
          <ul className="space-y-2.5 text-xs">
            ...
            <li>
              <a href={`mailto:${email}`} className="flex items-center gap-2 text-white/50 hover:text-amber-400 transition-colors break-all">
                <span className="text-amber-400/70">✉️</span>{email}
              </a>
            </li>
            <li className="flex items-start gap-2 text-white/45 leading-relaxed">
              <span className="text-amber-400/70 mt-0.5">📍</span><span>{address}</span>
            </li>
          </ul>
        </div>
      </div>
